...
function AdminGenericMotorbikeDetailPage() {
	const {
		...
		fetchGenericMotorbike,
		updateGenericMotorbike,
		importMotorbikeData,
	} = useGenericMotorbike()
	const { id } = useParams()
	...
	const handleSubmit = async (e: React.FormEvent) => {
		e.preventDefault()
		const mainImages = images.map((i) => ({
			imageResourceId: i.id,
			isGallery: false,
		}))
		const galleryImages = gallery.map((i) => ({
			imageResourceId: i.id,
			isGallery: true,
		}))
