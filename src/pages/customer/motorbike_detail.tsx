...
function MotorbikeDetailPage() {
    ...
    const { currentImageIndex, onMouseDown } = use360Image(currentGenericMotorbike?.images?.filter(image => !image.isGallery).length ?? 0) 
	const { id } = useParams()
    const [openDrawer, setOpenDrawer] = useState<'engine' | 'chassis' | 'warranty' | null>(null);

    const isInCart = useMemo(() => {
        return cart.some(item => item.genericMotorbikeId === currentGenericMotorbike?.id)
    ...
    return <div className="flex flex-col w-full gap-y-8">
        ...
        <div className="flex flex-col gap-y-4 p-8">
            <div className="flex gap-x-4">
                <div className="w-1/2" draggable={false} onMouseDown={onMouseDown}>
                    <img 
                        src={_imgLink(currentGenericMotorbike?.images[currentImageIndex]?.imageResource.s3Key ?? "")} 
                ...
                <div className="w-1/2 flex flex-col gap-y-4">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="text-2xl text-green-600 font-semibold">
                            ${currentGenericMotorbike?.recommendedPrice.toLocaleString()}
                        </div>
                        
                        {isInCart ? (
                            <button 
                                className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg flex items-center gap-2"
                                onClick={() => currentGenericMotorbike && removeFromCart(currentGenericMotorbike.id)}
                            >
                            ...
                            <button 
                                ...
                                onClick={() => currentGenericMotorbike && updateCart(currentGenericMotorbike.id, 1)}
                            >
                                Add to Cart
                            </button>
                        )}
                    ...
                    <div className="flex items-center gap-x-2">
                        <div 
                            ...
                            style={{ backgroundColor: currentGenericMotorbike?.colorInHex }}
                        />
